	<section class="feedback feedbackGray">
		<div class="feedback__inner  container">
			...
			global $title;
		        if($title == 0):
		      ?>
			<p class="feedback__heading section-heading">Закажите пропуск для <?php the_title();?> уже сейчас</p>
		      <?php else:?>
			<h2 class="feedback__heading section-heading">Закажите пропуск для <?php the_title();?> уже сейчас</h2>
		    <?php endif;?>
			<div class="feedback__form">
				<form action="" class="feedback__form-container" id = "feedbackform">
					...
					<div class="feedback__form-item feedback__form-item-btn action-btn uniSendBtn"  data-formid = "feedbackform" data-mailmsg = "Форма - Заказ пропуска для <?php the_title();?>">Оставить заявку</div>
				</form>
			</div>
		</div>
	</section> <!-- /.feedback -->